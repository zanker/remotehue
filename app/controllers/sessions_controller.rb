class SessionsController < ApplicationController
  ...
  def create
    ...
    # Create a new account for them silently
      ...
      Stats.batch.increment("signups/#{user.provider}")
    end
    ...
    Stats.batch.increment("user.logins/#{user.provider}")
    ...
    if new_account
      redirect_to usercp_bridges_path
    elsif env["omniauth.origin"] == "starter" or env["omniauth.origin"] == "premium"
      redirect_to CONFIG[:subscriptions][env["omniauth.origin"].to_sym][:url] % user._id.to_s
    elsif env["omniauth.origin"] and env["omniauth.origin"] =~ %r{^/}
      redirect_to env["omniauth.origin"], :notice => t("sessions.create.logged_in")
    else
      redirect_to usercp_bridges_path, :notice => t("sessions.create.logged_in")
    end
  end
